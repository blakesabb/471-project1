...
void EffectLimiter::Process(double* frameIn, double* frameOut)
{
	...
	for (int c = 0; c < 2; c++)
	{
		...
		if (wet > m_limit*m_limitMultiplier)
		{
			wet = m_limit*m_limitMultiplier;
		}
		frameOut[c] = dry + wet;
	}
...
void EffectLimiter::SetNote(CNote* note) {
	//m_note = note;
	...
	for (int i = 0; i < len; i++)
	{
		...
		else if (name == "limit")
		{
			//SetFreq(NoteToFrequency(value.bstrVal));
			value.ChangeType(VT_R8);
			m_limitMultiplier = value.dblVal;
		}
	}
